...
[data-testid="stFileUploaderLabel"] { display:none !important; }
...
with t6:
    ...
    if not sku_rows:
        ...
        if len(full_df) > 0:
            fs_show = full_df[["Store","Brand","Article No","Total Qty","Size Count","Sizes"]].sort_values(["Store","Brand"])
            st.dataframe(
        ...
        if len(cut_df2) > 0:
            cs_show = cut_df2[["Store","Brand","Article No","Total Qty","Size Count","Sizes"]].sort_values(["Store","Brand"])
            st.dataframe(
...
with t8:
    ...
    if dd_st:
        ...
        d3,d4 = st.columns(2)
        with d3:
            bd = ss.groupby('Brand')['NetSale'].sum().sort_values(ascending=False)
            ...
            fig_db.update_layout(**cl(280,"Brand Mix",margin=dict(l=10,r=10,t=50,b=60)),bargap=0.3,xaxis_tickangle=-30)
            st.plotly_chart(fig_db, use_container_width=True)

        with d4:
            gd = ss.groupby('Gender')['NetSale'].sum().sort_values(ascending=False)
